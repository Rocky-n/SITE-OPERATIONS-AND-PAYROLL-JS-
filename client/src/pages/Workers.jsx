...
export default function Workers() {
  ...
  const fetchData = async () => {
    try {
      ...
      setErrorMsg('Failed to load workers directory');
      toast.error('Failed to load workers directory');
      setLoading(false);
    }
  };
    ...
    <div className="p-8 space-y-6 max-w-7xl mx-auto">
      ...
      {loading ? (
        ...
        <div className="py-16 text-center bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 p-8">
          ...
          <h3 className="font-bold text-slate-700 dark:text-slate-200 text-base">No workers match criteria</h3>
            ...
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="divide-y divide-slate-100 dark:divide-slate-800 text-xs text-slate-700 dark:text-slate-300 font-medium">
                {filteredWorkers.map((w) => (
                  <tr key={w._id} className="hover:bg-orange-50/20 dark:hover:bg-orange-950/20 transition-colors">
                    <td className="py-4 px-6">
                      <div className="font-bold text-slate-900 dark:text-white">{w.name}</div>
                      <div className="text-[11px] text-slate-500 dark:text-slate-400">{w.role || 'General Worker'}</div>
                    </td>
                    ...
                    <td className="py-4 px-6 text-right">
                      <span className="font-bold text-slate-900 dark:text-white text-sm">₹{w.dailyWageRate?.toLocaleString()}</span>
